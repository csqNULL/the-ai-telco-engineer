...
class EC2RichardsonAPPDetector(Equalizer):
    ...
    def __init__(self, grid, *, num_bits_per_symbol=4, precision="single", device="cuda:0", **kwargs):
        ...
        one_idx = torch.stack([torch.nonzero(labels[:, kk], as_tuple=False).squeeze(-1) for kk in range(self.K)], dim=0)
        zero_idx = torch.stack([torch.nonzero(~labels[:, kk], as_tuple=False).squeeze(-1) for kk in range(self.K)], dim=0)
        self.one_idx_flat = one_idx
        self.zero_idx_flat = zero_idx
        self.one_idx = one_idx.view(1, 1, 1, self.K, -1)
        self.zero_idx = zero_idx.view(1, 1, 1, self.K, -1)
        self.point_pow = pts.real * pts.real + pts.imag * pts.imag
        self.points_r = pts.real.contiguous().view(1, 1, 1, self.q)
        self.points_i = pts.imag.contiguous().view(1, 1, 1, self.q)
        self.point_pow_view = self.point_pow.contiguous().view(1, 1, 1, self.q)

        dev = torch.device(device)
        self.row0 = torch.arange(self.M, dtype=torch.long, device=dev) * self.N
        self.mout_offset = (torch.arange(self.M, dtype=torch.long, device=dev) * (self.M * self.N)).view(1, self.M, 1)
    ...
    def _blocks_from_sparse(self, h_vals: torch.Tensor, col_idx: torch.Tensor) -> torch.Tensor:
        ...
        delta = torch.remainder(-ci, self.N)
        flat_idx = (self.mout_offset + lin * self.N + delta).reshape(b, -1)
    ...
    def _jacobi2_trace(self, offdiag_abs2: torch.Tensor, diag: torch.Tensor) -> torch.Tensor:
        d = diag.real.clamp_min(1.0e-8)
        invd = 1.0 / d
        corr = torch.sum(offdiag_abs2 * invd.unsqueeze(-2), dim=-1) * invd * invd
        v = (invd + corr).clamp_min(self.var_floor)
        return v.permute(0, 2, 1).mean(dim=-1).clamp_min(self.var_floor)

    def _linear_posterior(self, HhH: torch.Tensor, HhH_diag: torch.Tensor,
                          HhH_offdiag_abs2: torch.Tensor, Hhy: torch.Tensor,
                          pB: torch.Tensor, etaB: torch.Tensor):
        b = etaB.shape[0]
        rB = etaB / pB.unsqueeze(-1).to(etaB.dtype)
        rBf = torch.fft.fft(rB, dim=-1, norm="ortho").permute(0, 2, 1).contiguous()
        pB_n = pB.view(b, 1, self.M)
        A = HhH + self.eyeM * pB_n.unsqueeze(-1).to(HhH.dtype)
        rhs = Hhy + pB_n.to(Hhy.dtype) * rBf
        ...
        vlin_d = self._jacobi2_trace(HhH_offdiag_abs2, HhH_diag + pB_n)
        return xlin, vlin_d

    def _richardson_linear_half(self, HhH: torch.Tensor, HhH_diag: torch.Tensor,
                                HhH_offdiag_abs2: torch.Tensor, Hhy: torch.Tensor,
                                pB: torch.Tensor, etaB: torch.Tensor, seed_x: torch.Tensor):
        b = etaB.shape[0]
        rB = etaB / pB.unsqueeze(-1).to(etaB.dtype)
        rBf = torch.fft.fft(rB, dim=-1, norm="ortho").permute(0, 2, 1).contiguous()
        pB_n = pB.view(b, 1, self.M)
        ...
        diag_inv = (1.0 / diag_A.real.clamp_min(1.0e-8)).to(A.dtype)
        ...
        vlin_d = self._jacobi2_trace(HhH_offdiag_abs2, diag_A)
        return xlin, vlin_d

    def _qam_logp(self, r: torch.Tensor, prec_delay: torch.Tensor) -> torch.Tensor:
        p = prec_delay.unsqueeze(-1).unsqueeze(-1)
        dr = r.real.unsqueeze(-1) - self.points_r
        di = r.imag.unsqueeze(-1) - self.points_i
        return -p * (dr * dr + di * di)
    ...
    def _qam_project_from_logp(self, logp: torch.Tensor):
        ...
        mean_r = torch.sum(w * self.points_r, dim=-1) / z
        mean_i = torch.sum(w * self.points_i, dim=-1) / z
        mean = torch.complex(mean_r, mean_i)
        pow2 = torch.sum(w * self.point_pow_view, dim=-1) / z
    ...
    def _maxlog_llr_from_logp(self, logp: torch.Tensor) -> torch.Tensor:
        ...
        for kk in range(self.K):
            m1 = torch.amax(torch.index_select(logp, -1, self.one_idx_flat[kk]), dim=-1)
            m0 = torch.amax(torch.index_select(logp, -1, self.zero_idx_flat[kk]), dim=-1)
            llrs.append(m1 - m0)
    ...
    def _app_llr_from_logp(self, logp: torch.Tensor) -> torch.Tensor:
        llrs = []
        for kk in range(self.K):
            m1 = torch.logsumexp(torch.index_select(logp, -1, self.one_idx_flat[kk]), dim=-1)
            m0 = torch.logsumexp(torch.index_select(logp, -1, self.zero_idx_flat[kk]), dim=-1)
            llrs.append(m1 - m0)
        return torch.stack(llrs, dim=-1)
    ...
    def llr(self, y_dd: torch.Tensor, channel: ChannelView, no: torch.Tensor) -> torch.Tensor:
        ...
        HhH_diag = HhH.diagonal(dim1=-2, dim2=-1).real
        HhH_offdiag_abs2 = (HhH.real * HhH.real + HhH.imag * HhH.imag) * self.offdiag_mask
        ...
        for _ in range(self.full_iters):
            xlin, vlin_d = self._linear_posterior(
                HhH, HhH_diag, HhH_offdiag_abs2, Hhy, pB, etaB
            )
        ...
        xhalf, vhalf_d = self._richardson_linear_half(
            HhH, HhH_diag, HhH_offdiag_abs2, Hhy, pB, etaB, second_xmean
        )
